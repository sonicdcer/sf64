        ...
        if (objInit->id >= OBJ_EFFECT_339 && objInit->id <= OBJ_UNK_406) {
            switch (objInit->id) {
                ...
                case OBJ_UNK_405:
                    ...
                    break;
                case OBJ_UNK_402:
                    D_i6_801A7F78 = objInit->unk_E * 0.1f;
                    D_i6_801A7F60 = -(f32) objInit->unk_A;
                    break;
                case OBJ_UNK_400:
                    D_801782B8++;
                    break;
...
s32 func_8006351C(s32 index, Vec3f* pos, Vec3f* arg2, s32 arg3) {
    ...
    if ((arg3 == 0) || (arg3 == 2) || (arg3 == 3)) {
        ...
        for (i = 0; i < ARRAY_COUNT(gActors); i++, actor++) {
            if ((actor->obj.status >= 2) && (fabsf(pos->x - actor->obj.pos.x) < 1000.0f) &&
...
void Object_Init(s32 index, ObjectId objId) {
    ...
    switch (objId) {
        ...
        case OBJ_ACTOR_181:
            gActors[index].obj.rot.x = Rand_ZeroOne() * 360.0f;
            gActors[index].obj.rot.y = Rand_ZeroOne() * 360.0f;
            gActors[index].unk_0CE = 200;
            break;
        case OBJ_ACTOR_202:
            gActors[index].unk_0CE = 30;
            break;
        case OBJ_ACTOR_252:
        ...
        if (actor->unk_044) {
            if (D_800CFE5C[actor->unk_044] < 0.0f) {
                ...
                for (i = 0, otherActor = gActors; i < ARRAY_COUNT(gActors); i++, otherActor++) {
                    if ((otherActor->obj.status != 0) && (otherActor->index != actor->index) &&
...
void Actor_Update(Actor* actor) {
    ...
    switch (actor->obj.status) {
        ...
        case 2:
            func_800693E8(actor);
            if ((actor->obj.status != 0) && (actor->info.action != NULL)) {
