...
void func_8002E548(void) {
    if (D_8015F93C != 0) {
        ...
        if ((D_8015F930[0] == 0) && (sp24 != D_8015F930[1]) && (D_8015F930[1] < 15)) {
            AUDIO_PLAY_SFX(0x4900C02A, gDefaultSfxPos, 4);
        }
    }
}
    ...
    switch (player->unk_1D0) {
        ...
        case 2:
            if (D_80178340 == 255) {
                ...
                AUDIO_PLAY_SFX(0x4900200E, gDefaultSfxPos, 4);
                player->unk_1D0++;
            }
            break;
        case 3:
...
void func_8002F3E0(void) {
    ...
    for (i = 0, actor = &gActors[4]; i < 4; i++, actor++) {
        if (D_80177CD0[i] != 0) {
            ...
            Object_SetInfo(&actor->info, actor->obj.id);
            if (gCurrentLevel == LEVEL_VENOM_2) {
                AUDIO_PLAY_SFX(0x31004006, actor->sfxPos, 4);
            } else {
                AUDIO_PLAY_SFX(0x31004005, actor->sfxPos, 4);
            }
        }
    }
}
            ...
            for (i = 20, newActor = &gActors[20]; i < 60; i++, newActor++) {
                if (newActor->obj.status == OBJ_FREE) {
                    ...
                    if (gCurrentLevel == LEVEL_SECTOR_Z) {
                        ...
                        AUDIO_PLAY_SFX(0x09000002, newActor->sfxPos, 0);
    ...
    if (actor->unk_0D0 != 0) {
        ...
        if (actor->unk_0D0 != 0) {
            ...
            if (actor->health <= 0) {
                ...
                if (var_a1) {
                    AUDIO_PLAY_SFX(0x29001062, actor->sfxPos, 4);
                    actor->iwork[7] = 3;
                    actor->iwork[18] = 5;
                    actor->iwork[8] = 15;
                    if (Rand_ZeroOne() < 0.5f) {
...
void func_800319AC(Actor* this) {
    ...
    if ((this->unk_0E4 > 0) && (this->unk_0E4 < 4) && (gTeamShields[this->unk_0E4] <= 0) && (this->state != 6)) {
        ...
        if (this->timer_0C2 < 100) {
            ...
            this->fwork[7] = 360.0f;
            this->fwork[8] = 0.0f;
            AUDIO_PLAY_SFX(0x09000002, this->sfxPos, 0);
            this->unk_046 = 0;
        }
        gTeamShields[this->unk_0E4] = -1;
