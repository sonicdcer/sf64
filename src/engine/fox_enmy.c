...
bool func_enmy_80060FE4(Vec3f* arg0, f32 arg1) {
    ...
    if ((gLevelMode != LEVELMODE_ALL_RANGE) && (gPlayer[0].state_1C8 != PLAYERSTATE_LEVEL_INTRO)) {
        return true;
    }
...
void ActorMissileSeek_Update(Actor* this) {
    ...
    if ((Object_CheckCollision(this->index, &this->obj.pos, &sp8C, 1) != 0) || (this->dmgType != DMG_NONE) ||
        (this->obj.pos.y < (gGroundHeight + 10.0f)) || (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE)) {
...
void Item_CheckBounds(Item* this) {
    ...
    if ((gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) || (gPlayer[0].state_1C8 == PLAYERSTATE_STANDBY)) {
        Object_Kill(&this->obj, this->sfxSource);
    }
...
void Actor_Move(Actor* this) {
    ...
    if ((this->obj.id == OBJ_ACTOR_ZO_DODORA) || (gCurrentLevel == LEVEL_MACBETH) ||
        ...
        var_fv0 = 8000.0f;
    } else if (gPlayer[0].state_1C8 == PLAYERSTATE_ENTER_WARP_ZONE) {
        var_fv0 = 100000.0f;
    }
...
void TexturedLine_Update(TexturedLine* this) {
    ...
    if (gGameState == GSTATE_PLAY) {
        if (((this->mode == 1) || (this->mode == 101) || (this->mode == 50)) &&
            (gPlayer[0].state_1C8 == PLAYERSTATE_ACTIVE) && (gPlayer[0].hitTimer == 0)) {
...
void Object_Update(void) {
    ...
    gCullObjects = false;
    if ((gLevelMode == LEVELMODE_ON_RAILS) &&
        ((gPlayer[0].state_1C8 == PLAYERSTATE_INIT) || (gPlayer[0].state_1C8 == PLAYERSTATE_ACTIVE) ||
         (gPlayer[0].state_1C8 == PLAYERSTATE_DOWN) || (gPlayer[0].state_1C8 == PLAYERSTATE_ENTER_WARP_ZONE) ||
         (gPlayer[0].state_1C8 == PLAYERSTATE_START_360) || (gPlayer[0].state_1C8 == PLAYERSTATE_NEXT))) {
        gCullObjects = true;
    }
    if (gLevelMode != LEVELMODE_ALL_RANGE) {
        if ((gLoadLevelObjects != 0) && (gPlayer[0].state_1C8 != PLAYERSTATE_LEVEL_INTRO)) {
