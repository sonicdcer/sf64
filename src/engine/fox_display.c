...
void Display_DrawHelpAlert(void) {
    ...
    if ((gTeamHelpActor->obj.status != OBJ_ACTIVE) || (gPlayer[0].state_1C8 != PLAYERSTATE_ACTIVE)) {
        gTeamHelpActor = NULL;
        gTeamHelpTimer = 0;
        return;
    }
...
void Display_OnFootMuzzleFlash(Player* player) {
    ...
    if ((player->state_1C8 == PLAYERSTATE_ACTIVE) && (player->csTimer != 0)) {
...
void Display_LandmasterMuzzleFlash(Player* player) {
    ...
    if ((player->state_1C8 == PLAYERSTATE_ACTIVE) && (player->unk_1A0 != 0)) {
...
void Display_PlayerFeatures(Player* player) {
    ...
    if (player->draw && (player->state_1C8 != PLAYERSTATE_DOWN)) {
        switch (player->form) {
            case FORM_ARWING:
                Matrix_RotateY(gCalcMatrix, (player->yRot_114 + player->rot.y + player->damageShake + 180.0f) * M_DTOR,
                               MTXF_NEW);
...
void Display_ArwingWingTrail_Draw(Player* player) {
    ...
    if (player->contrailScale != 0.0f) {
        ...
        if (player->state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) {
            yRot = 0.0f;
            sp50 = 0.0f;
        }
...
void Display_Update(void) {
    ...
    if ((gCurrentLevel == LEVEL_AQUAS) && (gPlayer[0].state_1C8 == PLAYERSTATE_ACTIVE)) {
    ...
    } else if (camPlayer->state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) {
    ...
    if ((gLevelType == LEVELTYPE_PLANET) || (gCurrentLevel == LEVEL_BOLSE)) {
        if ((gCurrentLevel == LEVEL_TITANIA) &&
            ((gPlayer[0].state_1C8 != PLAYERSTATE_LEVEL_INTRO) || (gPlayer[0].unk_19C != 0))) {
            Matrix_Push(&gGfxMatrix);
            Matrix_Translate(gGfxMatrix, 0.0f, gCameraShakeY, 0.0f, MTXF_APPLY);
            Matrix_SetGfxMtx(&gMasterDisp);
    ...
    if ((gGroundSurface == SURFACE_WATER) && (gPlayer[0].state_1C8 != PLAYERSTATE_LEVEL_INTRO)) {
    ...
    if ((gLevelMode == LEVELMODE_TURRET) && (gPlayer[0].state_1C8 == PLAYERSTATE_ACTIVE)) {
        Turret_Draw(gPlayer);
    }
    ...
    if ((gCamCount != 1) &&
        ((camPlayer->state_1C8 == PLAYERSTATE_ACTIVE) || (camPlayer->state_1C8 == PLAYERSTATE_U_TURN))) {
        HUD_Draw();
