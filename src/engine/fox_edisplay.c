...
void ActorTeamArwing_Draw(ActorTeamArwing* this) {
    ...
    if ((((fabsf(dest.z) < 3000.0f) && (fabsf(dest.x) < 3000.0f) && !gBossActive) ||
         (gPlayer[0].state_1C8 == PLAYERSTATE_STANDBY) || (gCurrentLevel == LEVEL_VENOM_ANDROSS) ||
         (gCurrentLevel == LEVEL_VENOM_2) || (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE)) &&
        (gCurrentLevel != LEVEL_MACBETH) && (gCurrentLevel != LEVEL_TITANIA)) {
        if (this->obj.id == OBJ_ACTOR_CUTSCENE) {
            if (((gCurrentLevel == LEVEL_VENOM_2) && (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) &&
                 (this->index == 10)) ||
                ((gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) && (gPlayer[0].csState >= 100) &&
                 (gCurrentLevel == LEVEL_KATINA) && (this->index == 1)) ||
                ((gCurrentLevel == LEVEL_SECTOR_Y) && (gPlayer[0].state_1C8 == PLAYERSTATE_STANDBY) &&
                 (this->state == 5))) {
        ...
        if ((var_fv0 > sViewPos.z) && (sViewPos.z > var_fv1)) {
            if (fabsf(sViewPos.x) < (fabsf(sViewPos.z * 0.5f) + 500.0f)) {
                if (fabsf(sViewPos.y) < (fabsf(sViewPos.z * 0.5f) + 500.0f)) {
                    ...
                    this->info.draw(&this->obj);
                    sDrewActor = true;
                    if ((gPlayer[0].state_1C8 == PLAYERSTATE_ACTIVE) && (this->obj.id == OBJ_ACTOR_ALLRANGE) &&
        ...
        if (actor->obj.status >= OBJ_ACTIVE) {
            ...
            switch (gLevelMode) {
                ...
                case LEVELMODE_TURRET:
                    ...
                    if ((gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_INTRO) ||
                        (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_COMPLETE) || (gCurrentLevel == LEVEL_AQUAS)) {
...
void Effect_Draw(u8 arg0) {
    if (arg0 == 0) {
        gReflectY = 1;
        Effect_DrawAll(1);
    } else if ((gGroundSurface == SURFACE_WATER) && (gPlayer[0].state_1C8 != PLAYERSTATE_LEVEL_INTRO)) {
        gReflectY = -1;
        Matrix_Push(&gGfxMatrix);
        Matrix_Scale(gGfxMatrix, 1.0f, -1.0f, 1.0f, MTXF_APPLY);
        Effect_DrawAll(-1);
