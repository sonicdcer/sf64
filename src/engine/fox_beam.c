    ...
    if (sp60) {
        ...
        if (gLevelMode == LEVELMODE_ALL_RANGE) {
            for (i = 0, player = gPlayer; i < gCamCount; i++, player++) {
                ...
                test.z = test.x;
                if ((i != shot->sourceId) &&
                    ((player->state_1C8 == PLAYERSTATE_ACTIVE) || (player->state_1C8 == PLAYERSTATE_DOWN)) &&
...
void PlayerShot_DrawLaser(PlayerShot* shot) {
    ...
    if (gCamCount < 4) {
        ...
        if ((shot->unk_58 == 0) ||
            ((gCurrentLevel == LEVEL_METEO) && (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_INTRO))) {
            width = 4.0f;
            length = 20.0f;
        } else {
            width = 2.0f;
...
void PlayerShot_ApplyExplosionDamage(PlayerShot* shot, s32 damage) {
    ...
    if (gVersusMode) {
        ...
        for (i = 0; i < gCamCount; i++, player++) {
            if ((i != shot->sourceId) && (player->state_1C8 == PLAYERSTATE_ACTIVE) && (player->hitTimer == 0)) {
...
void PlayerShot_UpdateLockOnShot(PlayerShot* shot) {
    ...
    if ((shot->obj.pos.y < gGroundHeight) || (shot->timer == 1)) {
        ...
        if (shot->unk_60 == 0) {
            ...
            for (i = 0, player = gPlayer; i < gCamCount; i++, player++) {
                if (((player->state_1C8 == PLAYERSTATE_ACTIVE) || (player->state_1C8 == PLAYERSTATE_U_TURN)) &&
    ...
    switch (shot->obj.status) {
        ...
        case SHOT_ACTIVE:
            ...
            switch (shot->obj.id) {
                ...
                case PLAYERSHOT_SINGLE_LASER:
                case PLAYERSHOT_TWIN_LASER:
                    if ((shot->unk_58 == 0) || (gPlayer[0].state_1C8 == PLAYERSTATE_LEVEL_INTRO)) {
                        ticks = 4;
                    } else {
                        ticks = 3;
                    }
