...
void Training_ItemRing_Update(ItemTrainingRing* this) { // 60fps Training Ring
    ...
    switch (this->state) {
        case 0:
            ...
            Math_SmoothStepToF(&this->width, 1.0f, 1.0f DIV_FRAME_FACTOR, 0.05f DIV_FRAME_FACTOR,
                               0.0f); // 60fps training ring roation speed
        ...
        case 1:
            ...
            if (gPlayer[0].alternateView) {
                this->obj.pos.z += ((gPlayer[this->playerNum].trueZpos - 300.0f - this->obj.pos.z) * 0.05f)
                    DIV_FRAME_FACTOR; // 60fps ??????
            } else {
                this->obj.pos.z +=
                    (((gPlayer[this->playerNum].trueZpos - this->obj.pos.z) * 0.05f)) DIV_FRAME_FACTOR; // 60fps ??????
            }
            ...
            Math_SmoothStepToAngle(&this->obj.rot.y, Math_RadToDeg(-gPlayer[this->playerNum].camYaw),
                                   0.2f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR,
                                   0.0f); // 60fps ??????
