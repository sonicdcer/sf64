    ...
    if (D_8015F924 == 0) {
        ...
        for (actorPtr = gActors + 10, i = 10; i < 16; i++, actorPtr++) {
            if (actorPtr->obj.status == OBJ_FREE) {
                ...
                AUDIO_PLAY_SFX(0x3100000CU, actorPtr->sfxPos, 4U);
                break;
            }
        }
    }
    ...
    for (i = 10, actorPtr = &gActors[10]; i < arg1 + 10; i++, actorPtr++) {
        if (actorPtr->obj.status == OBJ_FREE) {
            ...
            if (D_i4_801A0530 > 1000) {
                ...
                AUDIO_PLAY_SFX(0x2903305FU, actorPtr->sfxPos, 4U);
    ...
    switch (actor->state) {
        ...
        case 2:
            ...
            if (gBosses[2].state == 10) {
                ...
                func_8002EE34();
                actor->state = 6;
                AUDIO_PLAY_SFX(0x31009063U, actor->sfxPos, 0U);
                AUDIO_PLAY_SFX(0x2940D09AU, actor->sfxPos, 4U);
                D_Timer_80161A60 = 8;
        ...
        case 10:
            ...
            if (!actor->timer_0BC) {
                ...
                func_800B7184(player, 1);
                player->unk_014 = 0.0f;
                Audio_KillSfxBySource(gBosses[1].sfxPos);
                actor->iwork[1] = gHitCount;
            }
            break;
...
bool func_i4_8018D584(Actor* actor) {
    ...
    if ((actor->unk_0D2 < 2) && (actor->state == 0)) {
        actor->timer_0C6 = 20;
        actor->unk_0D0 = 0;
        AUDIO_PLAY_SFX(0x29024003U, actor->sfxPos, 0U);
        ...
        Audio_KillSfxBySourceAndId(actor->sfxPos, 0x11000028U);
        AUDIO_PLAY_SFX(0x2903B009U, actor->sfxPos, 0U);
    } else {
...
void func_i4_8018ED44(void) {
    ...
    for (i = 0; i < ARRAY_COUNT(gActors); actor++, i++) {
        if (actor->obj.status == OBJ_FREE) {
            ...
            actor->vel.z = 200.0f;
            Object_SetInfo(&actor->info, actor->obj.id);
            AUDIO_PLAY_SFX(0x29002002U, actor->sfxPos, 4U);
            break;
        }
    }
}
    ...
    switch (player->unk_1D0) {
        ...
        case 1:
            ...
            if (player->timer_1F8 == 0) {
                ...
                AUDIO_PLAY_SFX(0x31009063U, actor50->sfxPos, 0U);
                func_8001C8B8(0);
            }
            break;
        ...
        case 10:
            ...
            switch (gCsFrameCount) {
                ...
                case 95:
                    Audio_KillSfxBySource(actor50->sfxPos);
                    AUDIO_PLAY_SFX(0x2902F026U, gActors[0].sfxPos, 0U);
...
void func_i4_80190D98(Effect* effect, f32 xPos, f32 yPos, f32 zPos, f32 xRot, f32 yRot) {
    ...
    effect->vel.z = dest.z;
    effect->timer_50 = 60;
    AUDIO_PLAY_SFX(0x31000013U, effect->sfxPos, 4U);
    ...
    switch (boss->state) {
        ...
        case 1:
            if (((gHitCount - gActors[0].iwork[1]) >= 5) || (D_i4_801A0530 > 8000)) {
                ...
                boss->timer_050 = 1000;
                AUDIO_PLAY_SFX(0x19034041U, boss->sfxPos, 0U);
            }
            break;
        ...
        case 2:
            ...
            if (boss->timer_050 == 1) {
                ...
                Audio_KillSfxBySourceAndId(boss->sfxPos, 0x19034041);
            }
            break;
        ...
        if (boss->dmgType != 0) {
            ...
            if (boss->dmgPart < 8) {
                ...
                if (boss->swork[boss->dmgPart] <= 0) {
                    ...
                    AUDIO_PLAY_SFX(0x2903B009U, boss->sfxPos, 4U);
                    boss->swork[36]--;
                    if (boss->swork[36] <= 0) {
                        boss->state = 10;
                        gBosses[0].state = 1;
                    ...
                    AUDIO_PLAY_SFX(0x29034003U, boss->sfxPos, 4U);
                }
            }
        }
...
void func_i4_80191BAC(Boss* boss) {
    ...
    switch (boss->state) {
        ...
        case 1:
            ...
            if (boss->timer_052 == 0) {
                boss->state = 2;
                AUDIO_PLAY_SFX(0x19401048U, boss->sfxPos, 0U);
            }
            break;
    }
    ...
    for (actor = &gActors[30], i = 0; i < 1000; i++) {
        ...
        if ((D_80178310[i].id >= OBJ_ACTOR_176) && (D_80178310[i].id <= OBJ_ACTOR_291)) {
            ...
            if ((actor->obj.id == OBJ_ACTOR_271) && (D_8015F924 == 0)) {
                AUDIO_PLAY_SFX(0x11000028U, actor->sfxPos, 0U);
            }
