...
#if ENABLE_60FPS == 1 // Fortuna_801875F0
...
#if ENABLE_60FPS == 1 // Fortuna_UpdateEvents
    ...
    switch (actor->state) {
        ...
        case 1:
            for (actorPtr = actor0 + 1, i = 0; i < 3; i++, actorPtr++) {
                ...
                if (gCsFrameCount == 264 MUL_FRAME_FACTOR) {
                    actorPtr->state = 2;
                    actor->state = 2;
        ...
        case 3:
            player->cam.eye.x += (actor4->vel.x * 0.23f) DIV_FRAME_FACTOR;
            player->cam.eye.y += (actor4->vel.y * 0.23f) DIV_FRAME_FACTOR;
            player->cam.eye.z += (actor4->vel.z * 0.23f) DIV_FRAME_FACTOR;
            Math_SmoothStepToF(&player->cam.at.x, actor4->obj.pos.x, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.y, actor4->obj.pos.y, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->cam.at.z, actor4->obj.pos.z, 1.0f DIV_FRAME_FACTOR, 20000.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->camRoll, 0.0f, 1.0f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0f);
        ...
        case 5:
            ...
            actor->fwork[0] += 10.0f DIV_FRAME_FACTOR;
...
#if ENABLE_60FPS == 1 // Fortuna_80188AD0
void Fortuna_80188AD0(Actor* actor) {

    actor->fwork[0] += 2.0f DIV_FRAME_FACTOR; // rotation of radar dish
    if (actor->state == 2) {
        actor->state = 3;
...
bool Fortuna_80188F08(s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3f* rot, void* ptr) { // Radar
    Actor* actor = (Actor*) ptr;
// msgPrint = "80188F08";
...
#if ENABLE_60FPS == 1 // Fortuna_LevelComplete
void Fortuna_LevelComplete(Player* player) {
    ...
    if ((player->csState < 10) && (player->csState >= 0)) {
        ...
        if (gMissionStatus == MISSION_COMPLETE) {
            ...
            Math_SmoothStepToF(&player->pos.y, 500.0f, 0.1f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f);
        }
    ...
    switch (player->csState) {
        case -1:
            ...
            player->rot.x = -7.0f;
            Math_SmoothStepToF(&player->rot.z, 0.0f, 0.05f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0f);
        ...
        case 0:
            if (gMissionStatus == MISSION_COMPLETE) {
                Math_SmoothStepToF(&player->rot.y, 40.0f, 0.1f DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(&player->rot.z, 60.0f, 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(&player->rot.x, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0.0f);
            } else {
                f32 tempAtan;
                f32 tempRagtoDeg;
                f32 tempSmooth;
                ...
                tempAtan = Math_Atan2F(player->pos.x, player->trueZpos);
                tempRagtoDeg = Math_RadToDeg(tempAtan);
                tempSmooth = Math_SmoothStepToF(&player->yRot_114, tempRagtoDeg, 0.1f DIV_FRAME_FACTOR, 4.0f DIV_FRAME_FACTOR, 0.0f);
                Math_SmoothStepToF(&player->rot.z, tempSmooth * 20.0f MUL_FRAME_FACTOR, 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f); // ???????
                Math_SmoothStepToF(&player->rot.x, 0.0f, 0.1f DIV_FRAME_FACTOR, 2.5f DIV_FRAME_FACTOR, 0.0f);
            }
        ...
        case 1:
            player->unk_190 = 2.0f;
            Math_SmoothStepToF(&player->rot.x, 15.0f, 0.1f DIV_FRAME_FACTOR, 0.4f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->rot.z, -40.0f, 0.2f DIV_FRAME_FACTOR, 5.0f DIV_FRAME_FACTOR, 0.0f);
            Math_SmoothStepToF(&player->rot.y, -120.0f, 0.1f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0.0f);
        ...
        case 10:
            ...
            if (gCsFrameCount == 496 MUL_FRAME_FACTOR) {
                player->csState = 11;
            ...
            if (gCsFrameCount == 420 MUL_FRAME_FACTOR) {
                gLevelClearScreenTimer = 100;
            }
            break;

        case 11:
            D_ctx_80177A48[1] += D_ctx_80177A48[3] DIV_FRAME_FACTOR;
            ...
            Math_SmoothStepToF(D_ctx_80177A48, 0.05f, 1.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR, 0);
            ...
            Math_SmoothStepToF(&player->baseSpeed, 0.0f, 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);

            Math_SmoothStepToF(&actor3->vel.z, 0.0f, 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor1->vel.z, 0.0f, 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor2->vel.z, 0.0f, 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);
            Math_SmoothStepToF(&actor0->vel.z, 0.0f, 0.05f DIV_FRAME_FACTOR, 2.0f DIV_FRAME_FACTOR, 0);

            if (gCsFrameCount == 500 MUL_FRAME_FACTOR) {
                Radio_PlayMessage(gMsg_ID_20010, RCID_FOX);
            }

            if (gMissionStatus == MISSION_COMPLETE) {
                switch (gCsFrameCount) {
                    case 588 MUL_FRAME_FACTOR:
            ...
            if (gCsFrameCount < 1136 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&D_ctx_80177A48[3], 0.55f, 1.0f DIV_FRAME_FACTOR, 0.02f DIV_FRAME_FACTOR, 0);
            } else {
                gStarfieldScrollY += 0.3f DIV_FRAME_FACTOR;
                gStarfieldScrollX += 0.3f DIV_FRAME_FACTOR;
                Math_SmoothStepToF(&D_ctx_80177A48[3], 0.0f, 1.0f DIV_FRAME_FACTOR, 0.02f DIV_FRAME_FACTOR, 0);
                if (gCsFrameCount == 1216 MUL_FRAME_FACTOR) {
                    player->csState = 12;
                    player->csTimer = 1000;
        ...
        case 12:
            ...
            gStarfieldScrollX += 0.3f DIV_FRAME_FACTOR;
            player->baseSpeed += 1.0f DIV_FRAME_FACTOR;
            player->baseSpeed *= IMPROPER_DIV_FRAME_FACTOR(1.15f);
            player->pos.y += D_ctx_80177A48[4] DIV_FRAME_FACTOR;
            D_ctx_80177A48[4] *= IMPROPER_DIV_FRAME_FACTOR(1.19f);
            player->unk_190 = 2.0f;
            ...
            if (gTeamShields[TEAM_ID_FALCO] > 0) {
                ...
                if (player->csTimer < 980) {
                    actor3->vel.z += 1.0f DIV_FRAME_FACTOR;
                    actor3->vel.z *= IMPROPER_DIV_FRAME_FACTOR(1.15f);
                    actor3->vel.y *= IMPROPER_DIV_FRAME_FACTOR(1.19f);
                    actor3->iwork[11] = 2;
                }
            }
            ...
            if (player->csTimer < 960) {
                actor2->vel.z += 1.0f DIV_FRAME_FACTOR;
                actor2->vel.z *= IMPROPER_DIV_FRAME_FACTOR(1.15f);
                actor2->vel.y *= IMPROPER_DIV_FRAME_FACTOR(1.19f);
                actor2->iwork[11] = 2;
            }
            ...
            if (gTeamShields[TEAM_ID_SLIPPY] > 0) {
                ...
                if (player->csTimer < 940) {
                    actor1->vel.z += 1.0f DIV_FRAME_FACTOR;
                    actor1->vel.z *= IMPROPER_DIV_FRAME_FACTOR(1.15f);
                    actor1->vel.y *= IMPROPER_DIV_FRAME_FACTOR(1.19f);
                    actor1->iwork[11] = 2;
                }
            }
            ...
            if (player->csTimer < 910) {
                actor0->vel.z += 1.0f DIV_FRAME_FACTOR;
                actor0->vel.z *= IMPROPER_DIV_FRAME_FACTOR(1.02f) ;
                actor0->vel.y *= IMPROPER_DIV_FRAME_FACTOR(1.06f) ;
                actor0->obj.rot.x *= IMPROPER_DIV_FRAME_FACTOR(1.03f);
            }

            if (gCsFrameCount == 1382 MUL_FRAME_FACTOR) {
        ...
        case 21:
            ...
            if (gCsFrameCount > 140 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor3->obj.rot.z, 0.0f, 0.03f DIV_FRAME_FACTOR, 1.2f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor3->obj.pos.x, -100.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor3->obj.pos.y, 40.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount > 180 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor1->obj.rot.z, 0.0f, 0.03f DIV_FRAME_FACTOR, 1.2f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor1->obj.pos.x, 100.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor1->obj.pos.y, 40.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount > 220 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor2->obj.pos.y, 90.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount > 340 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&player->pos.y, -10.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            }
            ...
            switch (gCsFrameCount) {
                case 140 MUL_FRAME_FACTOR:
                    AUDIO_PLAY_BGM(NA_BGM_COURSE_CLEAR);
                    break;
                case 450 MUL_FRAME_FACTOR:
                    gLevelClearScreenTimer = 100 MUL_FRAME_FACTOR;
                    break;
            }
            src.x = 0.0f;
            ...
            Math_SmoothStepToF(&gCsCamAtY, 0.0f, 0.005f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            player->cam.at.x = player->pos.x;
        ...
        case 22:
            if ((gCsFrameCount >= 1110 MUL_FRAME_FACTOR) && (gCsFrameCount < 1240 MUL_FRAME_FACTOR)) {
                Math_SmoothStepToF(&player->baseSpeed, 0.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor3->vel.z, 0.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor1->vel.z, 0.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor2->vel.z, 0.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor0->vel.z, 0.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount == 1239 MUL_FRAME_FACTOR) {
                player->baseSpeed = 0.0f;
                actor3->vel.z = 0.0f;
                actor1->vel.z = 0.0f;
                actor2->vel.z = 0.0f;
            ...
            switch (gCsFrameCount) {
                ...
                case 1080 MUL_FRAME_FACTOR:
                    gShowLevelClearStatusScreen = 1;
                    break;

                case 1240 MUL_FRAME_FACTOR:
                    if (gTeamShields[TEAM_ID_FALCO] > 0) {
                        AUDIO_PLAY_SFX(NA_SE_ARWING_BOOST, actor3->sfxSource, 0);
                        actor3->fwork[29] = 5.0f;
                        actor3->iwork[11] = 2;
                ...
                case 1300 MUL_FRAME_FACTOR:
                    AUDIO_PLAY_SFX(NA_SE_ARWING_BOOST, player->sfxSource, 0);
                    player->unk_190 = 2.0f;
                    player->unk_194 = 5.0f;
                    break;

                case 1350 MUL_FRAME_FACTOR:
                    SEQCMD_STOP_SEQUENCE(SEQ_PLAYER_BGM, 50);
                    SEQCMD_STOP_SEQUENCE(SEQ_PLAYER_FANFARE, 50);
                    break;
            }

            if ((gCsFrameCount >= 1400 MUL_FRAME_FACTOR)) {
                gFillScreenAlphaTarget = 255;
            ...
            if (gCsFrameCount > 1060 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(D_ctx_80177A48, 300.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&D_ctx_80177A48[1], -100.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            } else if (gCsFrameCount > 770 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(D_ctx_80177A48, 600.0f, 0.02f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            } else {
                Math_SmoothStepToF(D_ctx_80177A48, 400.0f, 0.03f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }

            Math_SmoothStepToF(&D_ctx_80177A48[5], D_ctx_80177A48[0], 0.01f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            Math_SmoothStepToF(&D_ctx_80177A48[7], D_ctx_80177A48[1], 0.01f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);

            if (gCsFrameCount > 1060 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&D_ctx_80177A48[2], -70.0f, 0.05f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            } else if (gCsFrameCount < 770 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&D_ctx_80177A48[2], 100.0f, 0.05f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }

            Math_SmoothStepToF(&D_ctx_80177A48[6], D_ctx_80177A48[2], 0.01f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);

            if (gCsFrameCount < 1180 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&D_ctx_80177A48[8], 0.41f, 0.01f DIV_FRAME_FACTOR, 10.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            } else {
                Math_SmoothStepToF(&D_ctx_80177A48[8], 0.0f, 0.001f DIV_FRAME_FACTOR, 1.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
            }

            D_ctx_80177A48[4] += D_ctx_80177A48[8] DIV_FRAME_FACTOR;
            ...
            if (gCsFrameCount >= 1240 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor3->vel.z, 50.0f, 0.1f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount >= 1260 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor1->vel.z, 50.0f, 0.1f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount >= 1280 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&actor2->vel.z, 50.0f, 0.1f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            if (gCsFrameCount >= 1300 MUL_FRAME_FACTOR) {
                Math_SmoothStepToF(&player->baseSpeed, 50.0f, 0.1f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
                Math_SmoothStepToF(&actor0->vel.z, 40.0f, 0.1f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.001f DIV_FRAME_FACTOR);
            }
            Math_SmoothStepToF(&gCsCamAtY, player->pos.y, 0.005f DIV_FRAME_FACTOR, 1000.0f DIV_FRAME_FACTOR, 0.0001f DIV_FRAME_FACTOR);
    ...
    if (player->csState < 20) {
        switch (gCsFrameCount) {
            case 1020 MUL_FRAME_FACTOR:
                gShowLevelClearStatusScreen = 1;
                break;
            case 1220 MUL_FRAME_FACTOR:
                gShowLevelClearStatusScreen = 0;
                break;
        }
    }
