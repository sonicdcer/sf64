    ...
    if (D_i3_801C4190[3] == 0) {
        ...
        if ((fabsf(D_i3_801C4308[10]) >= 6.0f) && (gBosses[0].state >= 10) && (gBosses[0].obj.status == OBJ_ACTIVE) &&
            ...
            if ((sp8C != D_i3_801C4190[4]) && (D_i3_801C41B8[5] == 3.0f)) {
                AUDIO_PLAY_SFX(0x4900001B, gDefaultSfxPos, 4);
    ...
    if (player->pos.y < (gGroundLevel + 50.0f)) {
        if (sp58 <= 0.0f) {
            ...
            if (D_i3_801C4190[7] == 0) {
                AUDIO_PLAY_SFX(0x9404028, player->sfxPos, 0);
                D_i3_801C4190[7] = 1;
            }
        }
    } else {
...
void func_i3_801AC09C(Player* player) {
    ...
    for (i = 15, shot = &gPlayerShots[15]; i < 16; i++, shot++) {
        if (shot->obj.status == OBJ_FREE) {
            ...
            AUDIO_PLAY_SFX(0x1000025, shot->sfxPos, 0);
            D_i3_801C4190[5] = i + 1;
            D_i3_801C4454 = 300;
    ...
    for (i = 0; i < 3; i++) {
        if (gPlayerShots[i].obj.status == OBJ_FREE) {
            ...
            if (gLaserStrength[gPlayerNum] == LASERS_SINGLE) {
                AUDIO_PLAY_SFX(0x9400021, gPlayerShots[i].sfxPos, 0);
            } else {
                AUDIO_PLAY_SFX(0x9400027, gPlayerShots[i].sfxPos, 0);
            }
            break;
        }
    }
...
void func_i3_801ACE50(Player* player) {
    ...
    if (player->timer_27C != 0) {
        ...
        if ((gBoostButton[player->num] & gInputHold->button) && (player->unk_230 == 0) &&
            ...
            if (player->unk_2BC == 0) {
                AUDIO_PLAY_SFX(0x9004030, player->sfxPos, 4);
            }
            player->unk_2BC += 3.0f;
...
void func_i3_801AD328(Player* player) {
    ...
    if ((gBrakeButton[player->num] & gInputHold->button) && (player->unk_230 == 0) &&
        (player->state_1C8 != PLAYERSTATE_1C8_5) && (player->unk_2B4 == 0)) {
        if (player->unk_2BC == 0) {
            AUDIO_PLAY_SFX(0x9004031, player->sfxPos, 4);
        }
        player->unk_2BC += 3.0f;
        if (player->unk_2BC > 90.0f) {
            player->unk_2BC = 90.0f;
    ...
    switch (actor->state) {
        case 0:
            if (actor->timer_0BC != 0) {
                ...
                actor->health = 30;
                AUDIO_PLAY_SFX(0x31000047, actor->sfxPos, 4);
                actor->state++;
            }
            break;
        case 1:
            if (actor->unk_0D0 != 0) {
                if (actor->health > 0) {
                    ...
                    AUDIO_PLAY_SFX(0x29034045, actor->sfxPos, 4);
                    if (actor->health <= 0) {
                        actor->timer_0C2 = 30000;
                        actor->state = 2;
                        actor->vel.y = 0.0f;
                        actor->vel.x = 0.0f;
                        actor->vel.z = 0.0f;
                        AUDIO_PLAY_SFX(0x2902A046, actor->sfxPos, 4);
                    }
                }
            }
        ...
        case 2:
            ...
            if (actor->timer_0BC == 0) {
                ...
                AUDIO_PLAY_SFX(0x29018049, actor->sfxPos, 4);
            }
            break;
    }
        ...
        case 0:
            ...
            if (actor->unk_0D0 != 0) {
                ...
                if ((fabsf(D_i3_801C4308[10]) >= 6.0f) && (gBosses[0].state >= 10) &&
                    ...
                    AUDIO_PLAY_SFX(0x29036050, actor->sfxPos, 4);
                    ...
                    if (D_i3_801C4308[10] <= -23.0f) {
                        ...
                        if (gBosses[0].swork[AQ_SWK_0] == 0) {
                            ...
                            if (actor->health <= 0) {
                                ...
                                AUDIO_PLAY_SFX(0x19000033, actor->sfxPos, 4);
                                actor->state++;
                            }
                        }
                    }
...
void func_i3_801AFA5C(Actor* actor) {
    ...
    if (actor->unk_0D0 != 0) {
        ...
        if ((gBosses[0].state >= 10) && (actor->health != 0) && (actor->damage == 31)) {
            actor->timer_0BE = 70;
            actor->health -= actor->damage;
            AUDIO_PLAY_SFX(0x2903604CU, actor->sfxPos, 4U);
            if (actor->timer_0C6 == 0) {
                actor->timer_0C6 = 70;
            }
            if (actor->health <= 0) {
    ...
    switch (actor->state) {
        ...
        case 3:
            ...
            if ((actor->iwork[1] == 13) && (i < 0)) {
                AUDIO_PLAY_SFX(0x29022048U, actor->sfxPos, 4U);
        ...
        case 4:
            ...
            actor->vel.y = 10.0f;
            actor->vel.z = 5.0f;
            AUDIO_PLAY_SFX(0x1902102FU, actor->sfxPos, 4U);
            break;
        case 5:
            if (actor->obj.pos.y > (gGroundLevel + 70.0f)) {
                ...
                if (actor->vel.z != 0) {
                    ...
                    AUDIO_PLAY_SFX(0x19402031U, actor->sfxPos, 4U);
                }
                if (actor->timer_0C0 == 1) {
                    AUDIO_PLAY_SFX(0x19003030U, actor->sfxPos, 4U);
                }
...
#pragma GLOBAL_ASM("asm/us/nonmatchings/overlays/ovl_i3/fox_aq/func_i3_801AFA5C.s")
#endif
...
void func_i3_801B099C(Actor* actor) {
    ...
    switch (actor->iwork[0]) {
        case 0:
            gSPDisplayList(gMasterDisp++, D_6019E80);
            break;
        case 1:
            gSPDisplayList(gMasterDisp++, D_60194D0);
            break;
        case 2:
            gSPDisplayList(gMasterDisp++, D_6019880);
            break;
    }
}
...
void func_i3_801B0EC0(Actor* actor) {
    if (actor->timer_0BC == 0) {
        ...
        Matrix_SetGfxMtx(&gMasterDisp);
        gSPDisplayList(gMasterDisp++, D_6024A50);
    }
}
...
void func_i3_801B10F8(Boss* bossAQ) {
    ...
    AUDIO_PLAY_SFX(0x3140904D, bossAQ->sfxPos, 4);
}
    ...
    switch (bossAQ->state) {
        ...
        case 1:
            if (bossAQ->timer_056 == 0) {
                ...
                bossAQ->timer_056 = 20;
                bossAQ->state = 2;
                AUDIO_PLAY_SFX(0x1900002E, bossAQ->sfxPos, 0);
            }
            break;
        case 2:
        ...
        case 14:
            if (bossAQ->timer_056 == 58) {
                AUDIO_PLAY_SFX(0x29408055, bossAQ->sfxPos, 4);
            }
        ...
        case 16:
            ...
            for (i3 = 0; i3 < 3; i3++) {
                func_80081A8C(bossAQ->obj.pos.x + RAND_FLOAT_CENTERED(1200.0f),
                              ...
                              bossAQ->obj.pos.z + 1000.0f + RAND_FLOAT_CENTERED(800.0f), 3.0f + RAND_FLOAT(3.0f), 5);
            }
            if (bossAQ->timer_056 == 0) {
    ...
    switch (bossAQ->swork[AQ_SWK_0]) {
        ...
        case 3:
            ...
            if (fabsf(D_i3_801C4308[10]) <= 5.0f) {
                ...
                bossAQ->swork[AQ_SWK_19] = 500;
                D_i3_801C4308[79] = 0.0f;
                AUDIO_PLAY_SFX(0x29408054, bossAQ->sfxPos, 4);
                if (bossAQ->state < 14) {
                    ...
                    D_80178480 = 50;
                    AUDIO_PLAY_SFX(0x1900002D, bossAQ->sfxPos, 0);
                    ...
                    for (i2 = 0; i2 < 3; i2++, var_fs3 += 10.0f) {
                        for (i3 = 0; i3 < 9; i3++) {
                            ...
                            func_8007B8F8(bossAQ->obj.pos.x + var_fs0, temp3, bossAQ->obj.pos.z + 1000.0f + var_fs1,
                                          20.0f);
                        }
                    }
                }
                ...
                case 13:
                    if ((bossAQ->swork[AQ_SWK_12] != 0) && (bossAQ->state >= 15)) {
                        if (D_i3_801C42A0[23] == 0) {
                            ...
                            AUDIO_PLAY_SFX(0x2940604C, bossAQ->sfxPos, 4);
                        }
                    } else if (bossAQ->state >= 10) {
                        AUDIO_PLAY_SFX(0x29121007, bossAQ->sfxPos, 4);
                    }
                    break;
                case 14:
                    if (bossAQ->damage == 31) {
                        if (D_i3_801C42A0[23] != 0) {
                            ...
                            AUDIO_PLAY_SFX(0x2903604C, bossAQ->sfxPos, 4);
                ...
                case 18:
                    if ((bossAQ->damage == 31) && (bossAQ->swork[AQ_SWK_1] != 2) && (bossAQ->swork[AQ_SWK_12] == 0) &&
                        ...
                        if (D_i3_801C42A0[23] != 0) {
                            ...
                            AUDIO_PLAY_SFX(0x2940804E, bossAQ->sfxPos, 4);
                            ...
                            if (bossAQ->health <= 0) {
                                ...
                                bossAQ->swork[AQ_SWK_1] = 2;
                                bossAQ->timer_056 = 50;
                                AUDIO_PLAY_SFX(0x2940A04F, bossAQ->sfxPos, 4);
    ...
    for (i2 = 45, i3 = 1, i7 = 0; i7 < 14; i3++, i2++, i7 += 2) {
        if (D_i3_801C42A0[i3] != 0) {
            D_i3_801C4308[i2] = SIN_DEG(D_i3_801C0060[i7 + 0] * D_i3_801C42A0[i3]) *
                                func_i3_801A958C(D_i3_801C42A0[i3], D_i3_801C0060[i7 + 1]);
            D_i3_801C42A0[i3]--;
        }
    }
    if (bossAQ->swork[AQ_SWK_12] != 0) {
        ...
        if (bossAQ->swork[AQ_SWK_13] != 0) {
            ...
            if (bossAQ->swork[AQ_SWK_13] == 1) {
                AUDIO_PLAY_SFX(0x29409051, bossAQ->sfxPos, 4);
            }
...
#else
void func_i3_801B134C(Boss* bossAQ);
#pragma GLOBAL_ASM("asm/us/nonmatchings/overlays/ovl_i3/fox_aq/func_i3_801B134C.s")
#endif
...
void func_i3_801B638C(Actor* actor) {
    ...
    if (actor->health == 0) {
        ...
        func_80066254(actor);
        Object_Kill(&actor->obj, actor->sfxPos);
        AUDIO_PLAY_SFX(0x1903001D, actor->sfxPos, 4);
        func_8007A6F0(&actor->obj.pos, 0x29038090);
    }
    ...
    if (actor->unk_0D0 != 0) {
        ...
        if ((actor->health != 0) && (actor->unk_0D2 == 0)) {
            actor->health -= actor->damage;
            actor->timer_0C6 = 30;
            AUDIO_PLAY_SFX(0x2903408F, actor->sfxPos, 4);
            if (actor->health <= 0) {
                actor->health = 0;
                BonusText_Display(actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z + 200.0f, 3);
                gHitCount += 4;
...
#pragma GLOBAL_ASM("asm/us/nonmatchings/overlays/ovl_i3/fox_aq/func_i3_801B638C.s")
#endif
...
#else
#pragma GLOBAL_ASM("asm/us/nonmatchings/overlays/ovl_i3/fox_aq/func_i3_801B6FF8.s")
#endif
    ...
    switch (actor->state) {
        ...
        case 3:
            if (actor->timer_0BE == 0) {
                ...
                actor->timer_0C0 = 10;
                AUDIO_PLAY_SFX(0x2900208D, actor->sfxPos, 4);
                actor->state = 4;
            }
            break;
        case 4:
            ...
            if ((actor->unk_0B6 < 21) && (actor->state == 4) && (actor->iwork[3] != 0)) {
                ...
                if (actor->unk_0B6 == 20) {
                    ...
                    effect->timer_50 = 100;
                    actor->iwork[3] = 0;
                    AUDIO_PLAY_SFX(0x2900308C, actor->sfxPos, 4);
                } else {
...
void func_i3_801B91A4(Actor* actor) {
    ...
    switch (actor->state) {
        ...
        case 2:
            ...
            if (actor->unk_0B6 >= 40) {
                func_i3_801AC8A8(actor->obj.pos.x + RAND_FLOAT_CENTERED(70.0f),
    ...
    switch (actor->state) {
        case 0:
            if (actor->iwork[1] == 0) {
                ...
                if (!(gGameFrameCount & 1)) {
                    func_i3_801AC8A8(actor->obj.pos.x + RAND_FLOAT_CENTERED(120.0f),
                ...
                if (actor->obj.pos.y < (gGroundLevel + 30.0f)) {
                    AUDIO_PLAY_SFX(0x19400007, actor->sfxPos, 4);
                    actor->iwork[1] = 1;
                    actor->vel.x = actor->vel.y = actor->vel.z = 0.0f;
                    actor->gravity = 0.0f;
...
void func_i3_801BAD7C(Actor* actor) {
    ...
    Matrix_SetGfxMtx(&gMasterDisp);
    gSPDisplayList(gMasterDisp++, D_6014FD0);
}
...
void func_i3_801BB79C(Actor* actor) {
    ...
    if (!(gGameFrameCount & 3)) {
        func_8007C120(actor->obj.pos.x + RAND_FLOAT_CENTERED(100.0f), actor->obj.pos.y + RAND_FLOAT_CENTERED(100.0f),
            ...
            for (i = 0; i < 2; i++) {
                for (j = 0; j < 60; j++) {
                    if ((gActors[j].obj.status == OBJ_ACTIVE) && (gActors[j].obj.id == OBJ_ACTOR_189) &&
                        (gActors[j].state == 58)) {
                        Object_Kill(&gActors[j].obj, gActors[j].sfxPos);
                        break;
                    }
                }
            }
            ...
            case 0:
                if (actor->state != 0) {
                    if ((actor->iwork[3] == 0) && (actor->info.drawType == 2)) {
                        ...
                        gSPDisplayList(gMasterDisp++, D_6014520);
                    }
                }
                break;
            case 1:
                gSPDisplayList(gMasterDisp++, D_60137F0);
                break;
            case 2:
                gSPDisplayList(gMasterDisp++, D_6014030);
                break;
        }
    }
}
...
void func_i3_801BD54C(Actor* actor) {
    ...
    if (actor->state == 2) {
        ...
        if (fabsf(actor->fwork[5] - actor->obj.pos.z) >= var_fs0) {
            actor->state = 3;
            if (actor->iwork[0] == 0) {
                AUDIO_PLAY_SFX(0x19400077, actor->sfxPos, 4);
            }
        } else {
            // needed to match
        }
        ...
        if (actor->health != 0) {
            if (actor->state == 2) {
                ...
                if (actor->health <= 0) {
                    ...
                    for (i = 0; i < 3; i++) {
                        func_80081A8C(actor->obj.pos.x + RAND_FLOAT(15.0f), actor->obj.pos.y + RAND_FLOAT(3.0f),
...
void func_i3_801BEB1C(Actor* actor) {
    ...
    if (actor->health != 0) {
        ...
        Matrix_SetGfxMtx(&gMasterDisp);
        gSPDisplayList(gMasterDisp++, D_6002C10);
    }
}
