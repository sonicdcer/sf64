...
void Andross_AndBrainWaste_Update(Actor* actor) {
    ...
    if (actor->dmgType != DMG_NONE) {
        ...
        Effect_Effect384_Spawn(actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z, 3.0f, 5);
...
void Andross_801888F4(Actor* actor) {
    ...
    if ((actor->timer_0BC == 0) || (actor->dmgType != DMG_NONE)) {
        Effect386_Spawn1(actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z, actor->vel.x, actor->vel.y, actor->vel.z,
                         7.0f, 20);
        Effect_Effect384_Spawn(actor->obj.pos.x, actor->obj.pos.y, actor->obj.pos.z, 10.0f, 5);
                ...
                case 2:
                    if (boss->swork[4] == 0) {
                        ...
                        if ((boss->health != 0) && (boss->health <= 0)) {
                            ...
                            gCsFrameCount = 0;
                            Audio_KillSfxBySourceAndId(boss->sfxSource, NA_SE_EN_ANDROSS_BRAIN);
                            Audio_KillSfxBySourceAndId(boss->sfxSource, NA_SE_EN_ANDROSS_CATCH);
                ...
                default:
                    if (boss->swork[4] == 0) {
                        ...
                        if (boss->swork[8] >= 6) {
                            ...
                            AUDIO_PLAY_SFX(NA_SE_EN_ANDROSS_WARP, boss->sfxSource, 4);
                        }
                    }
                    break;
            }
...
void Andross_AndExplosion_Update(Actor* actor) {
    ...
    if ((fabsf(actor->obj.pos.x - gPlayer[0].pos.x) < 2000.0f) &&
        ...
        for (i = 0; i < 1; i++) {
            Effect_FireSmoke_Spawn2(actor->obj.pos.x + displacement.x, actor->obj.pos.y + displacement.y,
...
void Andross_AndBrain_Update(Boss* boss) {
    ...
    if ((fabsf(boss->obj.pos.x - gPlayer[0].pos.x) < 300.0f) &&
        ...
        boss->state = 11;
        boss->timer_050 = 150;
        AUDIO_PLAY_SFX(NA_SE_EN_ANDROSS_CATCH, boss->sfxSource, 4);
    }
    ...
    switch (boss->state) {
        case 0:
            ...
            boss->swork[5] = 300;
            boss->state++;
            AUDIO_PLAY_SFX(NA_SE_EN_ANDROSS_BRAIN, boss->sfxSource, 4);
        ...
        case 20:
            ...
            Effect_FireSmoke_Spawn2(boss->obj.pos.x + sp64.x, boss->obj.pos.y + sp64.y, boss->obj.pos.z + sp64.z, 0.0f,
                                    0.0f, 0.0f, RAND_FLOAT(5.0f) + 7.0f);
            if (((gGameFrameCount % 2) == 0)) {
                Effect_Effect389_Spawn(RAND_FLOAT_CENTERED(600.0f) + boss->obj.pos.x,
                                       RAND_FLOAT(100.0f) + (boss->obj.pos.y + 100.0f), boss->obj.pos.z, 0.0f, 0.0f,
            ...
            if (gCsFrameCount == 200) {
                ...
                boss->state = 21;
                Effect_Effect384_Spawn(boss->obj.pos.x, boss->obj.pos.y, boss->obj.pos.z, 60.0f, 5);
            ...
            if (boss->state < 12) {
                ...
                if ((boss->state == 9) || (boss->state == 10)) {
                    if (boss->health <= 0) {
                        gScreenFlashTimer = 8;
                        AUDIO_PLAY_SFX(NA_SE_EN_DOWN_IMPACT, boss->sfxSource, 4);
                        AUDIO_PLAY_SFX(NA_SE_EN_ANDROSS_EXPLOSION, boss->sfxSource, 4);
                        AUDIO_PLAY_SFX(NA_SE_VO_ANDROSS_DEATH, boss->sfxSource, 4);
                        Audio_KillSfxBySourceAndId(boss->sfxSource, NA_SE_EN_ANDROSS_ROBOT);
                        SEQCMD_STOP_SEQUENCE(SEQ_PLAYER_BGM, 1);
                        SEQCMD_STOP_SEQUENCE(SEQ_PLAYER_FANFARE, 1);
                        boss->state = 31;
                        boss->timer_050 = 200;
                ...
                if (boss->swork[7] == 0) {
                    ...
                    if (boss->dmgPart == 2) {
                        ...
                        if (boss->swork[4] < 0) {
                            ...
                            Effect386_Spawn1(boss->vwork[0].x, boss->vwork[0].y, boss->vwork[0].z, 0.0f, 0.0f, 0.0f,
                                             20.0f, 15);

                            for (i = 0; i < 30; i++) {
                                Effect_Effect357_Spawn50(boss->vwork[0].x + RAND_FLOAT_CENTERED(500.0f),
                                                         boss->vwork[0].y + RAND_FLOAT_CENTERED(500.0f),
                                                         boss->vwork[0].z, RAND_FLOAT(1.0f) + 1.0f);
                            }
                            Effect_Effect384_Spawn(boss->vwork[0].x, boss->vwork[0].y, boss->vwork[0].z, 20.0f, 5);
                        ...
                        if (boss->swork[5] < 0) {
                            ...
                            Effect386_Spawn1(boss->vwork[1].x, boss->vwork[1].y, boss->vwork[1].z, 0.0f, 0.0f, 0.0f,
                                             20.0f, 15);

                            for (i = 0; i < 30; i++) {
                                Effect_Effect357_Spawn50(boss->vwork[1].x + RAND_FLOAT_CENTERED(500.0f),
                                                         boss->vwork[1].y + RAND_FLOAT_CENTERED(500.0f),
                                                         boss->vwork[1].z, RAND_FLOAT(1.0f) + 1.0f);
                            }
                            Effect_Effect384_Spawn(boss->vwork[1].x, boss->vwork[1].y, boss->vwork[1].z, 20.0f, 5);
                            Effect_SpawnTimedSfxAtPos(&sp68, NA_SE_EN_EXPLOSION_M);
                        } else {
                            Effect_SpawnTimedSfxAtPos(&sp68, NA_SE_OB_DAMAGE_M);
                        }
...
void Andross_AndAndross_Update(Boss* boss) {
    ...
    switch (boss->state) {
        ...
        case 6:
            if ((boss->swork[4] < 0) || (boss->swork[5] < 0)) {
                ...
                if (boss->timer_050 == 1) {
                    AUDIO_PLAY_SFX(NA_SE_VO_ANDROSS_WHAND, boss->sfxSource, 4);
                }
                if (boss->timer_050 != 0) {
                    boss->fwork[6] = -4000.0f;
                    boss->fwork[8] = 0.05f;
            ...
            switch (boss->unk_044) {
                ...
                case 1:
                    if (boss->timer_050 != 0) {
                        ...
                        boss->timer_050 = 70;
                        boss->fwork[9] = 0.0f;
                        AUDIO_PLAY_SFX(NA_SE_VO_ANDROSS_EXCITE, boss->sfxSource, 4);
                    }
        ...
        case 30:
            ...
            Math_SmoothStepToVec3fArray(spD0, D_i6_801A7F80, 1, limbCount, boss->fwork[9], 100.0f, 0.0f);
            if (((gGameFrameCount % 2) == 0)) {
                Effect_Effect389_Spawn(boss->obj.pos.x + RAND_FLOAT_CENTERED(1000.0f),
            ...
            switch (gCsFrameCount) {
                ...
                case 120:
                    ...
                    boss->timer_050 = 50;
                    boss->fwork[9] = 0.0f;
                    AUDIO_PLAY_SFX(NA_SE_EN_ANDROSS_ROBOT, boss->sfxSource, 4);
        ...
        case 9:
            ...
            Effect_Effect389_Spawn(boss->obj.pos.x + RAND_FLOAT_CENTERED(700.0f),
        ...
        case 32:
            ...
            switch (gCsFrameCount) {
                ...
                case 173:
                    ...
                    Effect_Effect384_Spawn(boss->obj.pos.x, boss->obj.pos.y, boss->obj.pos.z, 100.0f, 4);
                    break;
                case 180:
                    boss1 = &gBosses[1];
    ...
    if ((initialstate == 17) && (boss->state != 17)) {
        Audio_KillSfxBySourceAndId(boss->sfxSource, NA_SE_EN_ANDROSS_BREATH);
    }
    if ((initialstate == 7) && (boss->state != 7)) {
        Audio_KillSfxBySourceAndId(boss->sfxSource, NA_SE_EN_ANDROSS_SPARK);
    }
}
